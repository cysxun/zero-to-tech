...
const port = 6500 + Math.floor(Math.random()*50);
...
try {
  ...
  await send('Page.navigate', { url: 'http://localhost:80/' });
  await sleep(10000);
  ...
  console.log('config:', await evalJs('JSON.stringify(window.difyChatbotConfig||null)'));
  console.log('按钮:', await evalJs(`!!document.getElementById('dify-chatbot-bubble-button')`));
  console.log('embed 标签:', await evalJs(`Array.from(document.querySelectorAll('script[src*="embed"]')).map(s=>s.src).join(',')||'无'`));
